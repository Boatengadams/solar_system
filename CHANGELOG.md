...
## 1.0.0 — BAGS_LAB portable product

- Renamed the user-facing product identity to **BAGS_LAB** (scientific
  BAGSOLAR codebase preserved; internal `bag` / `bagsolar` identifiers retained
  where renaming would add risk).
- Strengthened executable-relative resource discovery for USB / arbitrary paths
  (`data/`, `assets/`, legacy `share/bagsolar` and new `share/bags_lab`).
- Added portable launchers under `launcher/` and a Linux packaging workflow
  (`tools/package_portable.sh` → `dist/linux-x64/`), including a dereferenced
  bundled `libraylib` so USB hosts do not need a system raylib install.

- Reorganized documentation into final project docs plus `docs/history/` for
  completed phase/agent planning records.
- Cleaned `.gitignore` for a professional development repository (docs and
  assets remain tracked; credentials and build trees stay ignored).
- Kept SPICE/CSPICE optional: core laboratory runs offline without external
  kernels and does not claim SPICE precision when disabled.
- Preserved physics, validation, telemetry, education, curriculum, spacecraft,
  missions, planet assets, and 3D rendering.
...
- Added the project MIT license and a third-party software/external-data
  summary.
- Identified historical planning docs for archival under `docs/history/` and

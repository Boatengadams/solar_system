...
## Phase 11.3/11.4 3D simulation renderer

The Simulation view uses raylib `Camera3D` and a presentation-only
`RenderTransform`. Physical positions remain SI metres in the simulation; the
renderer maps astronomical X to render X, astronomical Y to render Z/depth,
and astronomical Z to render Y/up at a deterministic display scale of 20
units per astronomical unit. Physical radii and visual radii are separate.

Right-drag orbits the camera, middle-drag pans, the mouse wheel zooms, `C`
focuses the selected body, and `HOME` resets the overview. A left click casts
a ray through the current camera and selects the nearest intersected display
sphere; empty space clears the selection. Bodies, highlights, orbit paths,
trails, and vectors all use the same world transform. The current body position
remains authoritative, while orbit paths use the existing heliocentric planar
simplification because the bundled dataset does not provide full authoritative
3D orbital orientation for every body.

The renderer does not calculate or modify physics, telemetry, ephemerides,
education state, spacecraft state, or mission state. Acceleration vectors are
not displayed because the current `Body` presentation state exposes velocity
but no authoritative per-body acceleration field.
